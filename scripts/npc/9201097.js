...
eQuestPrizes[4] = new Array ([0,250000]);	// Mesos
eQuestPrizes[5] = new Array ([0,250000]);	// Mesos
eQuestPrizes[6] = new Array ([0,250000]);	// Mesos
eQuestPrizes[7] = new Array ([0,250000]);	// Mesos
    ...
    if (status == 0) { // first interaction with NPC
		...
		if(!cm.haveItem(requiredItem,qnt)){
			cm.sendOk("Hmmm... are you sure you have #b" + qnt + " #t" + requiredItem + "##k? If so, then please check and see if your item inventory is full or not.");
							...
							cm.getPlayer().getInventory(Packages.client.inventory.MapleInventoryType.EQUIP).isFull(0)) {
			cm.sendOk("Your use or equip or etc inventory seems to be full. You need the free spaces to trade with me! Make room, and then find me.");
		} else if(prizeItem == 0) {
