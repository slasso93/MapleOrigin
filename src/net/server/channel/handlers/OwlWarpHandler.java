                ...
                if (GameConstants.isFreeMarketRoom(mapid)) {
                    if (hm.getChannel() == c.getChannel()) {
                        ...
                        if (hm.isOpen()) {   //change map has a delay, must double check
                            if (!hm.sameLeague(c.getPlayer())) {
                                c.getPlayer().dropMessage(1, "You cannot visit a " + (c.getPlayer().hasGroup() ? "standard" : "league") + " store on a " + (c.getPlayer().hasGroup() ? "league" : "standard") + " character.");
                                c.getPlayer().announce(MaplePacketCreator.enableActions());
                            } else if (hm.addVisitor(c.getPlayer())) {
