...
public class ItemCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        Item it = ItemConstants.getInventoryType(itemId).equals(MapleInventoryType.EQUIP) ?
                ii.getEquipById(itemId) : // equip
                new Item(itemId, (byte) 0, quantity); // non equip

        // set untradable and name tag if not admin
        short flag = 0;
        if (player.gmLevel() < 4) { // less than Super GM, set untradable
            flag |= ItemConstants.UNTRADEABLE;
        }
        if (player.gmLevel() < 6) { // less than Admin, also set name
