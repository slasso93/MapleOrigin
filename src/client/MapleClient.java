...
public class MapleClient {
    ...
    public Calendar getTempBanCalendarFromDB() {
        ...
        try {
            ...
            String blubb = rs.getString("tempban");

            if (blubb.startsWith("0000-00-00 00:00") || blubb.startsWith("2018-06-20 00:00")) { // 0000-00-00 or 2018-06-20 (default set in LoginPasswordHandler)
                return null;
            }
    ...
    private boolean isNameReserved(String name) {
        ...
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT id FROM namereserve where name = ?")) {
                ps.setString(1, name);
                try (ResultSet rs = ps.executeQuery()) {
                    reserved = rs.next();
                }
    ...
    public boolean canUseReservedName(String name) {
        boolean canUseReservedName = false;
        try (Connection con = DatabaseConnection.getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT a.id FROM accounts a join namereserve n on n.accountid=a.id where n.name= ?")) {
                ps.setString(1, name);
                try (ResultSet rs = ps.executeQuery()) {
                    canUseReservedName = !rs.next() || rs.getInt("id") == getAccID();
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return canUseReservedName;
    }

    public boolean reserveName(String name) {
        boolean reservedName = false;
        if (MapleCharacter.validNameString(name) && !MapleCharacter.isBannedName(name) && !isNameReserved(name)) {
            try (Connection con = DatabaseConnection.getConnection()) {
                try (PreparedStatement ps = con.prepareStatement("INSERT INTO namereserve VALUES (DEFAULT, ?, ?)")) {
                    ps.setInt(1, getAccID());
                    ps.setString(2, name);
                    reservedName = ps.executeUpdate() > 0;
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public boolean updateReservedName(String name) {
        ...
        if (MapleCharacter.validNameString(name) && !MapleCharacter.isBannedName(name) && !isNameReserved(name)) {
            try (Connection con = DatabaseConnection.getConnection()) {
                try (PreparedStatement ps = con.prepareStatement("UPDATE namereserve SET name= ?, accountid= ? where accountid IN (SELECT id FROM accounts where hwid= ?)")) {
                    ps.setString(1, name);
                    ps.setInt(2, accId);
                    ps.setString(3, convertHWID(getHWID()));
