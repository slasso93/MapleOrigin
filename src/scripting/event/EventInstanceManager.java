...
public class EventInstanceManager {
    ...
    public void exitPlayer(MapleCharacter chr, int map) {
        ...
        if (chr == this.getLeader()) {
            ...
            if (!this.getPlayers().isEmpty()) {
                ...
                System.out.println("Player Name: " + this.getLeader().getName());
            }
        } else {
            this.unregisterPlayer(chr);
            chr.changeMap(map, 0);
    ...
    public MapleCharacter getRandomPlayer() {
        ...
        MapleCharacter mc = players.get(0);
        return mc;
    }
    ...
    public void changeMusic(String value) {
        ...
        for (MapleCharacter chr : this.getPlayers()) {
            chr.getClient().announce(MaplePacketCreator.musicChange(value));
        }
    }
    ...
    public void registerRaid(MapleCharacter leader) {
        if (leader.isPartyLeader()) {
            registerParty(leader.getParty(), leader.getMap());
        }
    }

    public void registerRaid(MapleRaid raid, MapleMap map) {
    ...
    // return (MapleLifeFactory.getMonster(mid).getRank(mid));
    // }
